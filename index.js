...
function buildRewritePrompt(before, target, after, phrases) {
    ...
    return [
        ...
        "",
        "Principle: a direct synonym swap is best when it reads naturally; if it would be awkward, rephrase the banned reference minimally (e.g. into a pronoun or short description) instead of rewriting the whole sentence. Examples below use placeholder banned words for illustration only:",
        '  Banned "X": 낡은 X를 만지작거렸다 → 낡은 물건을 만지작거렸다',
        '  Banned "Y": Y라고 불리는 게 싫었다 → 그렇게 불리는 게 싫었다',
        "  (Each keeps the same sentence count, meaning, and length — only the banned word is resolved naturally.)",
        "",
